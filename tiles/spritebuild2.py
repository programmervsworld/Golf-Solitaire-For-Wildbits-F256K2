...
class SpriteCollection(object):
	...
	#
	#		Then 1,2,3 repeat for each sprite in the list, numbered sequentially.upda
	#
	def outputSprite(self,file = "graphics.bin"):
			h = open(file,"wb")
			# This is required by SB to tell it where the sprite block begins. It must be on $30000
			h.write(bytes([0x11]))							
			...
			for i in range(0,len(self.spriteList)):		
				...
				# This is where the 2x core gets messed up. On the first sprite there is an $11 in first
				# byte. This causes the sprites pixel 0,0 to be written in an even address. But afterwards
				# we are off by 1 byte each time and the next sprite needs to shift to an even byte before
				# writing the size byte down. 
				if i == 0:
					h.write(bytes([size]))
				else:
					h.write(bytes([0,size])) 
					
				# This is where the 32x32,16x16 or 8x8 data is written in rgb
				h.write(bytes(s.getData()))										

			# This tells SB the end of the sprite data blocks.
			h.write(bytes([0x80])) 	
			h.close()
